...
class Moora:
  def __init__(self, bobot , alternatif , label , desc):
    self.bobot = bobot
  ...
  def calculate_optimasi_multiobjecktif(self):
    ...
    for i in range(0,self.optimasi.shape[0]):
      self.rank[i] = abs(np.sum(self.optimasi[i,cost]) - np.sum(self.optimasi[i,ben]))
...
if __name__ == "__main__" :
  try:
    ...
    # desc = np.array([["Hamdi.,S.Pd"],["Purwanto.,S.Pd"],["L.Subhan.,S.Pd"],["Dewi Rosatika.,S.Pd"],["Tati Sunarti.,S.Pd"]])
    ...
    # desc = np.array([["A1"],["A2"],["A3"],["A4"],["A5"],["A6"],["A7"],["A8"],["A9"],["A0"]])
    ...
    desc = np.array([["Lado"],["Taro"],["Belinda"],["TM"],["Krispy"],["Tebing"],["Indra Putra"],["Keling"]])
    moora = Moora(bobot,alternatif,label,desc)
